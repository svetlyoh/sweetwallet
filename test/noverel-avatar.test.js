...
test('avatar artwork is contained and first-time wallet creation is a labeled action', () => {
	const html = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
	const css = fs.readFileSync(path.join(root, 'sweetwallet.css'), 'utf8');
	assert.match(html, /id="createWallet"[\s\S]*?Create New Wallet/);
	assert.match(html, /id="openImportFlow"[\s\S]*?I Already Have a Wallet/);
	assert.match(css, /\.login-avatar-preview img\s*\{[\s\S]*?object-fit: contain/);
	assert.match(css, /\.access-primary,[\s\S]*?\.access-secondary[\s\S]*?width: 100%/);
});

test('new wallet setup starts with the protected backup ceremony', () => {
	const html = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
	const client = fs.readFileSync(path.join(root, 'sweetwallet.js'), 'utf8');
	assert.match(html, /id="pinSetupBackupStep"[\s\S]*?I saved my private key somewhere safe/);
	assert.match(client, /\$\('#createWallet'\)[\s\S]*?openWallet\(keys, true\);[\s\S]*?openPinSetupFlow\(true\)/);
	assert.match(client, /state\.pinSetup\.newWallet[\s\S]*?setPinSetupStep\('complete'\)/);
});
