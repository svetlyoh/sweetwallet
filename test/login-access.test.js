...
test('saved startup and empty-browser startup choose usable login modes', () => {
	assert.equal(Access.initialLoginMode(savedWithPin), 'pin');
	assert.equal(Access.initialLoginMode(savedWithoutPin), 'password');
	assert.equal(Access.initialLoginMode(null), 'privateKey');
	...
	assert.equal(Access.loginPresentation('privateKey', null).submitVisible, true);
});
...
test('login mode switches expose the requested usable control and describe unavailable methods', () => {
	...
	const privateKey = Access.loginPresentation('privateKey', savedWithPin);
	...
	assert.equal(privateKey.secretVisible, true);
	assert.equal(privateKey.submitVisible, true);
